...
| 成果书 | 否 | 填金额、面议、咨询；空白或“无”表示没有该方案 |
| 任务书 | 否 | 填金额、面议、咨询；空白或“无”表示没有该方案 |
| PPT送答辩模板 | 否 | 填金额、面议、咨询；空白或“无”表示没有该方案 |
| 成果书+任务书+PPT+答辩模板+过AI+过查重 | 否 | 全套文档服务价格；空白或“无”表示没有该方案 |
| 是否展示 | 否 | 填“是”或“否”，默认为“是” |
| 排序 | 否 | 整数，越小越靠前 |

图片尺寸不要求一致。网站会使用固定画框完整显示，不裁剪、不拉伸；点击图片可查看原图。没有图片的项目不会显示空白图片区。

网站卡片的起价只按仿真、PCB和硬件三类设计方案计算，避免低价文档单项造成误解。价格组合默认展开并分为“设计与硬件”和“文档与答辩”，可自由勾选、填写备注并实时计算总价；选择全套文档服务时会自动取消重复的文档单项。确认需求后可一键复制项目名称、编号、需求组合、报价和备注，也可以填写姓名、手机号或微信号，以及需要邮寄时的地址并直接提交。空白或“无”的价格方案不会显示，也不参与计算；全部方案都不可用时不显示报价控件。

## 隐私说明

- 公开 Excel 不允许包含`下载链接`、`资料介绍链接`和`资料主要内容`等内部字段。
- 这些字段也不会进入 `projects.json`，网页不会展示任何下载地址或内部资料。
- 客户主动提交的姓名、手机号或微信号、邮寄地址和需求内容会写入 CloudBase 的 `orders` 集合，只用于需求确认、联系沟通和邮寄。
- `orders` 集合权限为 `ADMINONLY`，公开网页不能读取订单；客户信息不会写入网址或浏览器本地存储。
- 当前没有自动保留期限。订单处理完成且不再需要后，应由管理员在 CloudBase 控制台删除。
...
Windows 下可以双击 `上传工具/启动上传工具.bat`。工具会优先选择项目根目录的 `项目链接清单.xlsx`；检测通过后会把它复制到正式 `data` 目录并上传。有问题会在日志区域显示具体原因，检测通过后才能点击“上传到 GitHub”。工具只提交 `data/项目链接清单.xlsx`，不会提交其他文件。
...
直接打开 `site/index.html` 可以查看现有页面。要测试客户信息提交，请启动本地服务器：
...
本地服务器优先读取项目根目录的 `项目链接清单.xlsx`。保持服务器运行时，在 WPS 中保存该文件，服务器会自动重新解析，检测通过后浏览器会自动刷新；无需手工运行转换命令。
...
npm ci --prefix cloudbase/functions/submitOrder
node --test
```
